...
import { ProfileData } from '../types/team.types';
...
export function useProfile() {
  ...
  const handleUpdateProfile = async (setMessage: (msg: string) => void) => {
    try {
      const res = await fetch(`${API_BASE}/api/user/profile`, {
        method: 'PUT',
        headers: {
  ...
  const handleRegenerateApiKey = async (setMessage: (msg: string) => void) => {
  ...
  const handleSendBroadcast = async (selectedTeam: any, setMessage: (msg: string) => void, loadNotifications: (teamId: string) => void) => {
