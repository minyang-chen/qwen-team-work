...
export class AgentConfigManager implements IAgentDiscovery {
  ...
  private async loadConfiguration(): Promise<void> {
    try {
      ...
      this.agents = config.agents.sort((a: AgentConfig, b: AgentConfig) => 
  ...
  async selectBestAgent(capability: string | string[]): Promise<AgentConfig | null> {
  ...
  async registerAgent(agent: AgentConfig): Promise<void> {
    this.addAgent(agent as AgentConfig);
  }
}
