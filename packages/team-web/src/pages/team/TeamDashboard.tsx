...
export function TeamDashboard({ onTeamSelected, onLogout }: { onTeamSelected: () => void; onLogout?: () => void }) {
  const [activeTab, setActiveTab] = useState<TabType>('dashboard');
  const [workspaceType] = useState<WorkspaceType>('team');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {activeTab === 'task-assistant' ? (
        ...
        <>
          {/* @ts-ignore - Complex prop types */}
          <ProjectTab
            projectSubTab={projectSubTab}
            setProjectSubTab={setProjectSubTab}
            selectedTeam={teams.selectedTeam as any}
